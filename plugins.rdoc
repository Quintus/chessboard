...
a "reply_post_content" hook that allows you to insert HTML below the
text area used for replying to posts, creating topics, etc. In order
to do so, you define the method +hook_reply_post_content+ like this:
...
the hook. The +reply_post_content+ hook for example also adds an entry
